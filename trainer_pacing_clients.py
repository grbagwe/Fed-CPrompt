...
import copy
from trainer import federated_average
import gc
...
class Trainer:
    ...
    def __init__(self, args, seed, metric_keys, save_keys):
        ...
        self.pacing  = 1 # set pacing 2 for pacing =2
    ...
    def train(self, avg_metrics):
        ...
        client_task = {}
        for client in range(self.num_clients): # init task to clients
            client_task[client] = 0

        group_1 = {0,1,2,3,4}
        group_2 = {5,6,7,8,9}
        # group_1 = {0,1} # to debug
        # group_2 = {2, 3}
        ...
        for task_no in range(self.max_task): # cycle through the tasks # each task
            ...
            for round_ in range(self.num_rounds): # federated  comminication rounds
                ...
                # for client in range(4): # to debug
                    
                    self.client_learner[client].model.load_state_dict(self.global_model.state_dict())

                    if round_ == 0:
                        if client in group_1: # group 1
                            client_task[client] = task_no  
                        ...
                        elif client in group_2 and task_no == 0:
                            client_task[client] = task_no
                            # self.global_learner.last_valid_out_dim = self.global_learner.valid_out_dim
                            self.client_learner[client].last_valid_out_dim = self.client_learner[client].valid_out_dim # current valid dimension
                            self.client_learner[client].add_valid_output_dim(self.add_dim) # current valid dimension
                            ...
                            if task_no> 0 :
                                try:
                                    ...
                                    if self.client_learner[client].model is not None:
                                        self.client_learner[client].model.prompt.process_frequency()                    
                    elif round_ == int(self.num_rounds/2 ) and task_no < self.num_tasks -self.pacing:
                        if client in group_2:
                            client_task[client]  = task_no + self.pacing # pacing
                            p_task_no = task_no+ self.pacing  # pacing task
                            self.client_learner[client].last_valid_out_dim = p_task_no * 10 # current valid dimension
                            self.client_learner[client].valid_out_dim = (p_task_no+1) * 10 # current valid dimension
                            # print( f" ------------------group_2 {self.add_dim}--{task_no + 1}--------------\n \n  ")
                            # print( "  clients PREVIOUS valid out dimension ", self.client_learner[client].last_valid_out_dim)
                            ...
                            if task_no> 0 :
                                try:
                                    ...
                                    if self.client_learner[client].model is not None:
                                        self.client_learner[client].model.prompt.process_frequency()
                        if client in group_1 :
                            try :
                                if self.client_learner[client].model.module.prompt is not None:
                                    self.client_learner[client].model.module.prompt.next_task_locs_f([task_no * 10 , (task_no + 1) * 10])
                            except:
                                if self.client_learner[client].model is not None:
                                    self.client_learner[client].model.prompt.next_task_locs_f([task_no * 10 , (task_no + 1) * 10])     
                    ...
                    try:
                        self.client_learner[client].model.module.task_id = client_task[client]
                    except:
                        self.client_learner[client].model.task_id = client_task[client]

                    # print("which client",client_task[client])
                    self.train_dataset.load_dataset(t=client_task[client], train=True, client= client) # load client dataset
                    print( f" ------------------original task{task_no} ----- task : {client_task[client]}, round {round_}, client {client} -----------------")
                    print( "  clients PREVIOUS valid out dimension ", self.client_learner[client].last_valid_out_dim)

                    print( "  clients current valid out dimension ", self.client_learner[client].valid_out_dim)

                    if task_no == 0 and round_ < int(self.num_rounds/2 ) and client in group_1:
                        if client in group_1:
                            locs = np.isin(self.train_dataset.targets, self.train_dataset.tasks[task_no]).nonzero()[0]
                            train_dataset_sub = torch.utils.data.Subset(self.train_dataset, locs[:int(len(self.train_dataset.targets)/2)])
                            # load dataloader
                            if len(self.train_dataset.targets) < self.batch_size: # if dataste is smaller than the batch size in niid clients
                                ...
                                train_loader = DataLoader(train_dataset_sub, batch_size=temp_batch_size, shuffle=True, drop_last=True,
                                                    num_workers=int(self.workers)) # create train loader
                                print("subset 1 size = ", temp_batch_size * len(train_loader))
                            else:
                                train_loader = DataLoader(train_dataset_sub, batch_size=self.batch_size, shuffle=True, drop_last=True,
                                                    num_workers=int(self.workers))
                                print("subset 1 size = ", self.batch_size * len(train_loader))

                            print(np.unique(self.train_dataset.targets))
                            # loss , lweights , train_acc = self.client_learner[client].learn_batch(train_loader, train_dataset_sub) # learn the batch
                    ...
                    elif task_no == 0 and round_ >= int(self.num_rounds/2 ) and client in group_1:
                        if client in group_1:
                            locs = np.isin(self.train_dataset.targets, self.train_dataset.tasks[task_no]).nonzero()[0]
                            train_dataset_sub = torch.utils.data.Subset(self.train_dataset, locs[int(len(self.train_dataset.targets)/2):])
                            # load dataloader
                            if len(self.train_dataset.targets) < self.batch_size: # if dataste is smaller than the batch size in niid clients
                                ...
                                train_loader = DataLoader(train_dataset_sub, batch_size=temp_batch_size, shuffle=True, drop_last=True,
                                                    num_workers=int(self.workers)) # create train loader
                                print("subset 2 size = ", temp_batch_size * len(train_loader))

                            else:
                                train_loader = DataLoader(train_dataset_sub, batch_size=self.batch_size, shuffle=True, drop_last=True,
                                                    num_workers=int(self.workers))
                                print("subset 2 size = ", self.batch_size * len(train_loader))
            ...
            # *****************************
                        # loss , lweights , train_acc = self.client_learner[client].learn_batch(train_loader, self.train_dataset) # learn the batch
                        loss , lweights , train_acc = self.client_learner[client].learn_batch(train_loader, self.train_dataset, global_model =self.global_model , previous_task_model = self.previous_task_model) # learn the batch
                ...
                print("training loss : ", np.mean(agg_loss))
                print("training acc : ", np.mean(agg_train_acc))
                ...
                val_dataloader = DataLoader(self.val_dataset, batch_size=self.batch_size, shuffle=False, drop_last=False, num_workers=self.workers)
